...
//********信号定义***********************************************************
//关闭电源引脚，0：开电，1：关电
#define	SI4432SDN_PORT		PORT(0)
#define	SI4432SDN			BIT(6)

//SPI通信片选引脚
#define	SI4432NSEL_PORT		PORT(0)
#define	SI4432NSEL			BIT(5)

//中断输出引脚
#define	NIRQ_PORT			PORT(0)
#define	NIRQ				BIT(1)
...
void Si4432SetPacketHandler(SI4432PH *pSi4432PH) reentrant
{
	...
	//设置同步字内容
	Si4432_rw(SI4432_SYNC_WORD, (uint8 *)(&(pSi4432PH->SysWord)), 4, BWRITE);		//Address: 36h-39h — Synchronization Word 3-0

	//设置发送数据头
	Si4432_rw(SI4432_TX_HEADER, (uint8 *)(&(pSi4432PH->TxHeader)), 4, BWRITE);		//Address: 3Ah-3Dh — Transmit Header 3-0
	...
	//设置本地数据头
	Si4432_rw(SI4432_CHECK_HEADER, (uint8 *)(&(pSi4432PH->RxHeader)), 4, BWRITE);	//Address: 3Fh-42h — Check Header 3-0
	
	//设计接收数据头校验屏蔽位
	Si4432_rw(SI4432_HEADER_EN, (uint8 *)(&(pSi4432PH->RxHeaderCheckEn)), 4, BWRITE);	//Address: 43h-46h — Header Enable 3-0
...
void Si4432Send(uint8 *pd, uint16 nd) reentrant
{
	...
	//映射ID号===================================================================
	//中转器ID
	if((*((uint16*)pd) & RELAY_NET_ID) == RELAY_NET_ID)
	{
		nCodeType |= 0x20;
		*((uint16*)pd) &= ~RELAY_NET_ID;
	}													  
	else
	{
		nCodeType |= 0x10;
	...
	//采集器ID
	if((*((uint16*)pd) & COLLECTOR_NET_ID) == COLLECTOR_NET_ID)
	{
		nCodeType |= 0x08;
		*((uint16*)pd) &= ~COLLECTOR_NET_ID;
	}
	else
	{
		nCodeType |= 0x04;
...
uint8 Si4432Receive() reentrant
{
	...
	if(nSi4432Err == OS_NO_ERR) 
	{
		...
		if((nIntStatus16 & IS_VALID_PACKET_RX) != 0)				
		{
			...
			{
				*((uint16*)pSi4432RecvBuff) |= RELAY_NET_ID;
			}													  
		
			//采集器ID
			if((nCodeType & 0x08) == 0x08)
			{
				*((uint16*)pSi4432RecvBuff) |= COLLECTOR_NET_ID;
			}
